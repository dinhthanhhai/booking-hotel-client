    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      ListLinkHeader.forEach((item) => {
        ...
        if (item.note) {
          ...
          tempNote.className =
            "whitespace-nowrap rounded-sm bg-[#FF3366] text-white px-1 text-xs ml-1";
      ...
      <div className="mt-1 h-[56px] flex">
        <div className="w-full flex items-center justify-between">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="hidden text-xs text-[#FF3366] sm:block">
          ...
          <div className="hidden items-center gap-5 md:flex">
            ...
            <div className="hidden xl:flex gap-1 items-center">
              ...
              >
                ...
                <span className="text-xs text-[#FF3366]">Hoàn tiền tới 5%</span>
              </NavLink>
            </div>
        ...
        <div ref={containerRef} className="flex gap-4 relative h-full">
          {visibleItems.map((item, index) => (
            <NavLink
              ...
              to={`/${item.url}`}
              className={({ isActive }) =>
                "font-medium hover:text-[#FF3366] h-full pt-2 " +
                (isActive ? "text-[#FF3366] border-b-3 border-[#FF3366]" : "")
              }
            >
              <span className="whitespace-nowrap">{item.name}</span>
              {item?.note && (
                <span className="rounded-sm bg-[#FF3366] text-white px-1 text-xs h-fit ml-1">
                  {item?.note}
                </span>
              )}
            </NavLink>
                ...
                <div className="absolute w-[240px] right-0 top-10 bg-white shadow-sm rounded p-2 z-50">
                  {overflowItems.map((item) => (
                    <NavLink
                      ...
                      to={`/${item.url}`}
                      className={({ isActive }) =>
                        "block p-2 font-medium hover:text-[#FF3366] whitespace-nowrap" +
                        (isActive
                    ...
                    >
                      ...
                      {item?.note && (
                        <span className="rounded-sm bg-[#FF3366] text-white px-1 h-fit text-xs ml-1">
                          {item?.note}
                        </span>
                      )}
                    </NavLink>
